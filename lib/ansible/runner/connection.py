...
class Connector(object):
    ...
    def connect(self, host, port, user, password, transport, private_key_file, ssh_args):
        conn = utils.plugins.connection_loader.get(transport, self.runner, host, port, user=user, password=password, private_key_file=private_key_file, ssh_args=ssh_args)
        if conn is None:
